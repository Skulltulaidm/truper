...
# Configurar loggingimport streamlit as st
import pandas as pd
from typing import Dict, Set
import logging
from datetime import datetime
    ...
    if all([file_pedidos, file_inventarios, file_bd_brand]):
        try:
            ...
            with st.spinner("Cargando archivos..."):
                # Primero cargamos los archivos sin parsear fechas
                df_pedidos = pd.read_excel(file_pedidos)
                
                # Convertimos las columnas de fecha si existen
                date_columns = ['Embarque', 'Liberación']
                for col in date_columns:
                    if col in df_pedidos.columns:
                        try:
                            df_pedidos[col] = pd.to_datetime(df_pedidos[col], errors='coerce')
                        except Exception as e:
                            st.warning(f"No se pudo convertir la columna {col} a fecha. Error: {str(e)}")
                
                df_inventarios = pd.read_excel(file_inventarios)
                df_bd_brand = pd.read_excel(file_bd_brand)
                
                # Mostrar información de las columnas cargadas
                st.write("Columnas disponibles en el archivo de pedidos:", df_pedidos.columns.tolist())
